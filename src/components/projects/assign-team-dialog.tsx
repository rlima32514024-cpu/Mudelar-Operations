...
import { assignTeam, type ActionResult } from '@/app/actions/projects'
...
interface AssignTeamDialogProps {
  ...
  equipas: ResponsibleParty[]
}

export function AssignTeamDialog({ projectId, supervisors, equipas }: AssignTeamDialogProps) {
  const [open, setOpen] = useState(false)

  const boundAction = useCallback(
    (prev: ActionResult, formData: FormData) => assignTeam(projectId, prev, formData),
    [projectId]
  )
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-xl shadow-xl z-50 p-6">
          ...
          <form action={formAction} className="space-y-4">
            <div>
              <label htmlFor="at-supervisor" className="block text-sm font-medium text-gray-700 mb-1">
            ...
            <div>
              <label htmlFor="at-equipa" className="block text-sm font-medium text-gray-700 mb-1">
                Equipa de obras
              </label>
              <select
                id="at-equipa"
                name="equipa_obras_id"
                defaultValue=""
                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 bg-white"
              >
            ...
            <div>
              <label htmlFor="at-planned_start" className="block text-sm font-medium text-gray-700 mb-1">
                Data de arranque prevista
              </label>
              <input
                id="at-planned_start"
